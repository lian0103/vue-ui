...
var formatData = {
  woff: {
    extension: "woff",
    agent:
      "Mozilla/4.0 (Windows NT 6.2; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/32.0.1667.0 Safari/537.36",
    format: "woff",
  },
  woff2: {
    extension: "woff2",
...
if (!isGulp) {
  var cmd = require("commander")
    ...
    .option(
      ...
      "Name for output CSS file",
      defaultOptions.cssFilename
    )
    .option("--css-dir [path]", "CSS output directory", defaultOptions.fontsDir)
    .option(
      "--fonts-dir [path]",
      "Fonts output directory",
      defaultOptions.cssDir
    )
    .option(
      "--font-display-type [type]",
      "Font display type",
      defaultOptions.fontDisplayType
    )
    .option(
      "--out-base-dir [path]",
      "Base path to output directory, prepended to cssDir/fontsDir",
      defaultOptions.outBaseDir
    )
    .option(
      "--host [domain]",
      'Host to query ("fonts.googleapis.com")',
      defaultOptions.host
    )
    .option(
      "--host-path [path]",
      'Host path for query ("css")',
      defaultOptions.hostPath
    )
    .option(
      "--protocol [protocol]",
      "Protocol used for requests [https|http]",
      defaultOptions.protocol
    )
    .option(
      "--format [format]",
      "Format to retrieve [woff|woff2|svg|eot|ttf]",
  ...
  process.stdin
    ...
    .on("end", function () {
      getter(cmd)(src, null, function (err) {
        if (err) {
          throw err;
        } else {
          process.exit(0);
...
function getter(options) {
  ...
  if (isGulp) {
    if (options.outBaseDir) {
      throw new Error(
        "outBaseDir only valid when run from command line, use gulp.dest instead"
      );
    }
  }
  ...
  function processor(file, enc, next) {
    ...
    if (isGulp) {
      ...
      if (file.isStream()) {
        return self.emit(
          "error",
          new Error("webfont-getter: Streaming not supported")
        );
      }
    ...
    var param = data
      ...
      .replace(/^\|*|\|*$/g, "");
    async.waterfall(
      [initial(param), requestCss, receiveCss, parseCss, downloadFonts],
      next
    );
    ...
    function requestCss(param, next) {
      ...
      http.get(req, async.apply(next, null)).on("error", next);
    }
    ...
    function parseCss(css, next) {
      ...
      function extractData(block, ext) {
        ...
        function formatData(data) {
          ...
          var extt =
            src.split("/")[src.split("/").length - 1].split(ext)[0] + ext;
            var name = [family, style, weight].join("-") + "-" + extt;
    ...
    function generateFontCss(requests, classes, next) {
      ...
      var css = requests.map(makeFontFace).concat(classes).join("\n\n");
      writeFile(
      ...
      function makeFontFace(request) {
        var uri = path.posix.join(
          options.relativePaths
            ? path.posix.relative(options.cssDir, options.fontsDir)
            : options.fontsDir,
          request.name
        );
        request.src =
          request.locals.map((local) => local + ", ").join("") +
          "url(" +
          uri +
          ")";
      ...
      function downloadFont(request, next) {
        ...
        function emitFont(name, res, next) {
          if (res.statusCode !== 200) {
            next(
              new Error(
                "HTTP GET returned code " + res.statusCode + " for " + name
              )
            );
            return;
          }
